...
parser = argparse.ArgumentParser(add_help=False,
                                 description='Downloads sdf associated '
                                             'with the authenticated account.')
parser.add_argument('--file_path', default=('%s/sdf.csv'
                                            % os.path.dirname(
                                                os.path.realpath(__file__))),
                    help=('Path to the file you want to download the sdf '
                          'to.'))
parser.add_argument('--file_types', required=True,
                    help=('Filter by the file types. Provide them '
...
parser.add_argument('--filter_type', required=True,
                    help=('The type of filter you would like to use. Valid '
                          'options are: ADVERTISER_ID, CAMPAIGN_ID, INSERTION_ORDER_ID, INVENTORY_SOURCE_ID, LINE_ITEM_ID, PARTNER_ID'))


def main(doubleclick_bid_manager, file_path, body):
  # Construct the request.
  request = doubleclick_bid_manager.sdf().download(body=body)
  ...
  with open(file_path, 'wb') as handler:
    ...
    handler.write(request.execute()['insertionOrders'].encode('utf-8'))
    print('Download complete.')


if __name__ == '__main__':
  request_body = {}
  valid_file_types = ['AD', 'AD_GROUP', 'CAMPAIGN', 'INSERTION_ORDER', 'INVENTORY_SOURCE', 'LINE_ITEM']
  valid_filter_types = ['ADVERTISER_ID', 'CAMPAIGN_ID', 'INSERTION_ORDER_ID', 'INVENTORY_SOURCE_ID', 'LINE_ITEM_ID', 'PARTNER_ID']
  ...
  if args.file_types and args.file_types.upper() in valid_file_types:
    request_body['fileTypes'] = args.file_types.split(',')
  else:
    raise ValueError('Invalid fileType. Acceptable values: %s' %
                    valid_file_types)
